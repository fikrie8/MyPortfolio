...
const ExperienceCard = ({ exp, index }) => {
  ...
  return (
    <div className={`flex items-center justify-between w-full mb-12 md:mb-20 ${isEven ? 'md:flex-row-reverse' : 'flex-row'}`}>
      ...
      >
        <div className="p-5 sm:p-8 bg-white dark:bg-slate-800/40 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-2xl transition-all duration-500 group">
          ...
          {/* Job Title & Company */}
          <div className="mb-6">
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white group-hover:text-blue-600 transition-colors overflow-x-auto no-scrollbar pb-1">
              {exp.role}
            </h3>
            <p className="text-md font-bold text-blue-600 dark:text-blue-400 mt-1 uppercase tracking-tight overflow-x-auto no-scrollbar pb-1">
              {exp.company}
            </p>
          </div>
          ...
          <div className="space-y-4">
                ...
                <div className="overflow-hidden">
                    ...
                    <div className="space-y-2 border-l-2 border-slate-100 dark:border-slate-700 pl-4">
                      ...
                      // 1. Handle Spacers
                        ...
                        return (
                          <div key={i} className="flex items-start gap-2 text-sm text-slate-600 dark:text-slate-400 italic overflow-x-auto no-scrollbar pb-1">
                          <FiChevronRight 
                              className={`mt-1 shrink-0 ${isHeader ? 'text-blue-600' : 'text-blue-400/40'}`} 
                          />
                          <span className={`leading-relaxed whitespace-normal wrap-break-words ${
                              isHeader 
            ...
            <div>
              ...
              <ul className="space-y-2">
                {exp.jobScope.map((item, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-slate-600 dark:text-slate-400 overflow-x-auto no-scrollbar pb-1">
            ...
            <div className="p-5 bg-blue-50/30 dark:bg-blue-900/10 rounded-2xl border border-blue-100/50 dark:border-blue-800/30">
              <h4 className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest mb-3 overflow-x-auto no-scrollbar pb-1">Key Contributions</h4>
              <ul className="space-y-3">
                {exp.contribution.map((item, i) => (
                  <li key={i} className="flex items-start gap-3 text-sm font-medium text-slate-700 dark:text-slate-200 overflow-x-auto no-scrollbar pb-1">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {exp.techApplied.map((skill, i) => (
                  <span key={i} className="text-[10px] px-3 py-1.5 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 rounded-xl border border-slate-200 dark:border-slate-700 font-bold uppercase shadow-sm overflow-x-auto no-scrollbar pb-1">
                    {skill}
                  </span>
                ))}
              </div>
