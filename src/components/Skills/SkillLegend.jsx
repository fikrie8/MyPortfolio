        ...
        <div key={i} className="flex items-center gap-3">
          ...
          <div className="flex gap-0.5">
            {[1, 2, 3, 4, 5].map((step) => (
              <div 
                key={step}
                className={`h-1 w-3 rounded-sm transition-all duration-500 ${
          ...
          {/* Label text */}
          <span className="text-[9px] sm:text-[10x] uppercase tracking-wider font-black text-slate-500 dark:text-slate-400">
            {level.label}
          </span>
        </div>
      ))}
